...
typedef struct _uartInf
{
    ...
    stError              lastError;  /*!< error code of the last error */
...
stError uartCreate( uint8_t id, uint32_t rxBufferSize, uint32_t txBufferSize )
{
    if (id >= UART_MAX_NUMBER_OF_UARTS)
    {
...
/*******************************************************************************/
stError uartInitialize( uint8_t id, uint32_t clkSourceFrequency, uint32_t baudRate, uint32_t * realBaudRate )
{
    if (id >= UART_MAX_NUMBER_OF_UARTS)
    {
        return ERR_INSERT_UART_GRP(ERR_PARAM);
...
/*******************************************************************************/
stError uartDeinitialize( uint8_t id )
{
    if (id >= UART_MAX_NUMBER_OF_UARTS)
    {
        return ERR_INSERT_UART_GRP(ERR_PARAM);
...
uint32_t uartTxNBytes( uint8_t id, const uint8_t * buffer, uint32_t size )
{
    ...
    if( id == UART0 )
    {
        if( uartMaxTxBytes(id) > size ) 
        {
            ...
            memcpy( uart0TransmitDMABuffer, buffer,  size );
...
/*******************************************************************************/
stError uartLastError( uint8_t id )
{
    stError lastError;
    ...
    /* Check if there was an UART overrun which did not trigger an IRQ (e.g. during initialization) */
    if(  __HAL_UART_GET_FLAG( uartInfo[id].hUART, HAL_UART_ERROR_ORE) )
    {
        return ERR_INSERT_UART_GRP(ERR_HW_OVERRUN);
    }
