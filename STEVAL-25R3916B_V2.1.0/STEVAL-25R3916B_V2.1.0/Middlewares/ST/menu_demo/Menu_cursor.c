...
/** @brief Check the joystick direction to detect a change on the cursor, and update it accordingly
  * @param cursor Pointer on the cursor to check for an input
  * @param JoyState Pointer on a joystick direction
  * @retval 0 the cursor has not changed
  * @retval 1 the cursor has changed
  */
uint8_t Menu_CursorJoyInput(Menu_Cursor_t *cursor, Menu_Direction_t *JoyState)
{
  if(((JoyState->Dir == MENU_DOWN) || (JoyState->Dir == MENU_UP)) && (cursor->Bar->Direction == MENU_BAR_VERTICAL))
  {
    Menu_ClearCursor(cursor);
    if(JoyState->Dir == MENU_UP)
    {
      if((cursor->Y > cursor->minY) && (cursor->Value > cursor->Min))
      {
        cursor->Y--;
      }
    }
    ...
    {
      if((cursor->Y < cursor->maxY) && (cursor->Value < cursor->Max))
      {
        cursor->Y++;
      }
    }
  ...
  else if(((JoyState->Dir == MENU_LEFT) || (JoyState->Dir == MENU_RIGHT)) && (cursor->Bar->Direction == MENU_BAR_HORIZONTAL))
  {
    Menu_ClearCursor(cursor);
    if(JoyState->Dir == MENU_LEFT)
    {
      if((cursor->X > cursor->minX) && (cursor->Value > cursor->Min))
      {
        cursor->X--;
      }
    }
    
    if(JoyState->Dir == MENU_RIGHT)
    {
      if((cursor->X < cursor->maxX) && (cursor->Value < cursor->Max))
      {
        cursor->X++;
      }
    }
