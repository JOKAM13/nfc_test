...
void Menu_GetMaxIconeSize(uint16_t* MaxX, uint16_t* MaxY)
{
  *MaxX = *MaxY = 0;
  ...
  {
    if((psCurrentMenu->IconsList != NULL) && (psCurrentMenu->IconsList[i] != NULL))
    {
...
uint8_t Menu_SelectImage( const char *Picture[], const uint8_t nbPicture, const uint32_t *picture_size )
{
  ...
  {
    sprintf(msg,"%lu Bytes",(unsigned long) picture_size[0]);
    ...
    if( Select != 0 )
    {
      ...
      if( (Pos.X < 40) || (Pos.X > 280) )
      {
        ...
        if(update_picture == 1)
        {
          ...
          if(picture_size != NULL)
          {
            sprintf(msg,"%lu Bytes", (unsigned long) picture_size[idx]);
            Menu_DisplayCenterString(((centerline * 2) - 1), msg);
          }
       } /* update_picture */
      }
...
/**
  * @brief  Main loop to display menu.
