    ...
    if(Menu_ReadDirection(&JoyState))
    {
      if(JoyState.Dir == MENU_RIGHT)
      {
        if(tab.current != (tab.nbTab - 1))
        {
          ...
          Menu_DrawTabHeader(tab.x,tab.y,tab.width,tab.height,tab.tabRound,tab.tabHeight,tab.current,tab.nbTab);
          Menu_SetStyle(TAB);
          Menu_DrawTabHeader(tab.x,tab.y,tab.width,tab.height,tab.tabRound,tab.tabHeight,(tab.current + 1),tab.nbTab);
          Menu_SetStyle(PLAIN);
          Menu_DisplayTabName(&tab,(tab.current + 1));
