...
******************************************************************************
*/
#define READ_VAL08_LE(result, type, ptr)      (result = (type) ((ptr)[0]),   (ptr)  += sizeof( uint8_t ))
#define READ_VAL16_LE(result, type, ptr)      (result = (type) ((ptr)[0] + ((ptr)[1] << 8U)),     (ptr)  += sizeof( uint16_t ) )
#define READ_VAL32_LE(result, type, ptr)      (result = (type) ((ptr)[0] + ((ptr)[1] << 8U) + ((ptr)[2] << 16U) + ((ptr)[3] << 24U)),     (ptr)  += sizeof( uint32_t ) )
#define READ_BUF(result, size, ptr)  (RFAL_MEMCPY(result, (ptr), size),    (ptr)  += size)

#define WRITE_VAL08_LE(value, type, ptr)     ( * ((ptr) ++) = ( uint8_t ) (value & 0xFF) )
#define WRITE_VAL16_LE(value, type, ptr)     ( * ((ptr) ++) = ( uint8_t ) (value & 0xFF),\
                                               * ((ptr) ++) = ( uint8_t ) ((value >>  8)& 0xFF))
#define WRITE_VAL32_LE(value, type, ptr)     ( * ((ptr) ++) = ( uint8_t ) (value & 0xFF)         ,\
                                               * ((ptr) ++) = ( uint8_t ) ((value >>  8) & 0xFF) ,\
                                               * ((ptr) ++) = ( uint8_t ) ((value >> 16) & 0xFF) ,\
                                               * ((ptr) ++) = ( uint8_t ) ((value >> 24) & 0xFF))
#define WRITE_BUF(src, size, dest)  (RFAL_MEMCPY(dest, src, size),    dest  += size)


#define  ISODEP_DEVICE_INFO_STRUCT_SIZE  31U
#define  ISODEP_POLL_A_STRUCT_SIZE  (20U + ISODEP_DEVICE_INFO_STRUCT_SIZE)
#define  ISODEP_POLL_B_STRUCT_SIZE  (34U + ISODEP_DEVICE_INFO_STRUCT_SIZE)

#define PERSISTENT_ANALOG_CONFIG_WRITE 0
#define PERSISTENT_ANALOG_CONFIG_ERASE 1
#define PERSISTENT_ANALOG_CONFIG_CRC   2
...
enum rfalCommand
{
    ...
    RFAL_CMD_DPO_READ                          = 0x5A,
    RFAL_CMD_DPO_WRITE                         = 0x5B,
    RFAL_CMD_DPO_MEASURE_FUNC                  = 0x5C,
    RFAL_CMD_DPO_ADJUST_AND_MEASURE            = 0x5D,
    RFAL_CMD_DPO_SET_ENABLED                   = 0x5E,
    RFAL_CMD_DPO_IS_ENABLED                    = 0x5F,

    RFAL_CHIP_CMD_SET_RFO                      = 0x60,
    RFAL_CHIP_CMD_GET_RFO                      = 0x61,
    RFAL_CHIP_CMD_WAKEUP_START                 = 0x62,
    RFAL_CHIP_CMD_WAKEUP_HASWOKE               = 0x63,
    RFAL_CHIP_CMD_WAKEUP_GETINFO               = 0x64,
    RFAL_CHIP_CMD_WAKEUP_STOP                  = 0x65,

    RFAL_CMD_AC_SET                            = 0x66,
    RFAL_CMD_DPO_GET_CURRENT_TABLE_ENTRY       = 0x67U,    /*!< DPO Get current table index */
    
    RFAL_CMD_GET_TRANSCEIVE_RSSI               = 0x70,
};
enum IsodepCommand
{
	ISODEP_CMD_EXTENSION1                     = 0X100,
	ISODEP_CMD_INITIALIZE                     = ISODEP_CMD_EXTENSION1 + 0x00,
	ISODEP_CMD_INITIALIZEWITHPARAMS           = ISODEP_CMD_EXTENSION1 + 0x01,
	ISODEP_CMD_DESELECT                       = ISODEP_CMD_EXTENSION1 + 0x02,
	ISODEP_CMD_ATTRIB                         = ISODEP_CMD_EXTENSION1 + 0x03,
	ISODEP_CMD_FSXI2FSX                       = ISODEP_CMD_EXTENSION1 + 0x04,
	ISODEP_CMD_FWI2FWT                        = ISODEP_CMD_EXTENSION1 + 0x05,
	ISODEP_CMD_GETAPDUTRANSCEIVESTATUS        = ISODEP_CMD_EXTENSION1 + 0x06,
	ISODEP_CMD_GETMAXINFLEN                   = ISODEP_CMD_EXTENSION1 + 0x07,
	ISODEP_CMD_GETTRANSCEIVESTATUS            = ISODEP_CMD_EXTENSION1 + 0x08,
	ISODEP_CMD_ISATTRIB                       = ISODEP_CMD_EXTENSION1 + 0x09,
	ISODEP_CMD_ISRATS                         = ISODEP_CMD_EXTENSION1 + 0x0A,
	ISODEP_CMD_LISTENGETACTIVATIONSTATUS      = ISODEP_CMD_EXTENSION1 + 0x0B,
	ISODEP_CMD_LISTENSTARTACTIVATION          = ISODEP_CMD_EXTENSION1 + 0x0C,
	ISODEP_CMD_PPS                            = ISODEP_CMD_EXTENSION1 + 0x0D,
	ISODEP_CMD_POLLAHANDLEACTIVATION          = ISODEP_CMD_EXTENSION1 + 0x0E,
	ISODEP_CMD_POLLBHANDLEACTIVATION          = ISODEP_CMD_EXTENSION1 + 0x0F,
	ISODEP_CMD_POLLHANDLESPARAMETERS          = ISODEP_CMD_EXTENSION1 + 0x10,
	ISODEP_CMD_RATS                           = ISODEP_CMD_EXTENSION1 + 0x11,
	ISODEP_CMD_STARTAPDUTRANSCEIVE            = ISODEP_CMD_EXTENSION1 + 0x12,
	ISODEP_CMD_STARTTRANSCEIVE                = ISODEP_CMD_EXTENSION1 + 0x13,

    CMD_EXT_PERSISTENT_ANALOG_CONFIG          = ISODEP_CMD_EXTENSION1 + 0x21,
	ISODEP_CMD_COMMANDS                       = 0X7F,

    CMD_APPL_COM_WUPA                         = 0xE3, /*!< EMV TTA L1 WUPA button pressed. */
    CMD_APPL_COM_WUPB                         = 0xE4, /*!< EMV TTA L1 WUPB button pressed. */
...
extern uint8_t rfalDpoGetCurrentTableIndex( void );
...
static uint8_t                 gRxBuf[1024];             /* rx buffer used only for rfal non blocking TxRx or general functions */
static uint16_t                gRcvdLen;                 /* rx length used only for rfal non blocking TxRx */
// Use for Isodep
static rfalIsoDepApduBufFormat gIsoDepApduBuffer;        /* Buffer dedicated to apdu */
static bool                    gIsoDepTransceiveOngoing; /* On going flag for APDU tranceive */
static ReturnCode              gIsoDepTransceiveError;   /* status of APDU transceive        */
...
static ReturnCode processIsoDep       (const uint8_t *rxData, uint16_t rxSize, uint8_t *txData, uint16_t *txSize);
static ReturnCode processCardEmulation(const uint8_t *rxData, const uint16_t rxSize, uint8_t *txData, uint16_t *txSize);
static ReturnCode processDefault      (const uint8_t *rxData, const uint16_t rxSize, uint8_t *txData, uint16_t *txSize);
/*
******************************************************************************
* GLOBAL FUNCTIONS
******************************************************************************
...
uint8_t processProtocols ( const uint8_t * rxData, uint16_t rxSize, uint8_t * txData, uint16_t *txSize)
{
    ...
    uint8_t cmd = *rxData;
    ReturnCode err = RFAL_ERR_REQUEST;
    ...
    { /* default processing*/
        err = processDefault(rxData, rxSize, txData, txSize);
    }
...
/*!
  ...
  <li> 0x00: status 0 : RFAL_ERR_NONE
  </ul>
  ...
  - Enable/Disable RF field
    ...
    returns status RFAL_ERR_RF_COLLISION if field could not be turned on.
  ...
  -  RFAL Initialize
    <table>
     ...
     txSize must be >=2, returns status RFAL_ERR_NONE if no error was identified.
  ...
  -  RFAL Start Transceive
    <table>
     ...
     returns status RFAL_ERR_NONE if no error was identified.
  ...
  -  RFAL Get Transceive Status
    <table>
     ...
     txSize must be >=2, returns status RFAL_ERR_NONE if no error was identified and response is:
    <table>
  ...
  -  RFAL Transceive ISO14443A Short Frame
    <table>
     ...
     returns status RFAL_ERR_NONE if no error was identified and response is:
    <table>
      <tr><th>   Byte</th> <th>0..1 </th> <th> 2..2+rxLen </th></tr>
      <tr><th>Content</th> <td>rxLen</td> <td>rxData      </td> </tr>
    </table>
...
static uint8_t processCmd ( const uint8_t * rxData, uint16_t rxSize, uint8_t * txData, uint16_t *txSize)
{
    ...
    uint8_t err = (uint8_t)RFAL_ERR_REQUEST;
    ...
    if (cmd == 0x15) /* Process Direct commands */
    {
       err = processDirectCommand(buf, bufSize, txData, txSize);
    }
    if (cmd == 0x17) /* Set RFAL observation mode */
    {
        rfalSetObsvMode( buf[0], buf[1] );
        if (*txSize) *txSize = 0;
        err = RFAL_ERR_NONE;
    }
    if (cmd ==  0x18) /* ISO14443 tranceive */
    {
        uint32_t us;
        uint16_t actlength = 0;
        if (*txSize < 4) return (uint8_t)RFAL_ERR_PARAM;
        timerStopwatchStart();
    ...
    if (cmd == 0x19) /* tune antenna */  
    if (bufSize == 14)
    {
        tuningParams_t tuningParams;
        tuningStatus_t tuningStatus;
        
        tuningParams.mode             = (tuningAlgorithmMode_t)buf[0];
        tuningParams.AAT_A_first      = buf[1];
        tuningParams.AAT_B_first      = buf[2];
        tuningParams.AAT_A_last       = buf[3];
        tuningParams.AAT_B_last       = buf[4];
        tuningParams.AAT_A_stepWidth  = buf[5];
        tuningParams.AAT_B_stepWidth  = buf[6];
        tuningParams.phaTarget        = buf[7];
        tuningParams.phaWeight        = buf[8];
        tuningParams.ampTarget        = buf[9];
        tuningParams.ampWeight        = buf[10];

        timerStopwatchStart();
        err = tuneAntenna(&tuningParams, &tuningStatus);
        uint16_t duration = timerStopwatchMeasure();

        //
        txData[0] = (tuningStatus.AAT_A_tuned);
        txData[1] = (tuningStatus.AAT_B_tuned);
        txData[2] = (tuningStatus.phaTuned);
        txData[3] = (tuningStatus.ampTuned);
        txData[4] = 0;
        
        txData[6] = (tuningStatus.measureCnt) >> 8; 
        txData[5] = (tuningStatus.measureCnt);       
    ...
    if (0x19 == cmd) /* tune antenna - new rfal functions */
        if (15 <= bufSize)
    {
        struct st25r3916AatTuneParams tuningParams;
        struct st25r3916AatTuneResult tuningStatus;

        //unsigned char keepFieldOn     = st25r3916IsTxEnabled();
        tuningParams.aat_a_min        = buf[0];
        tuningParams.aat_a_max        = buf[1];
        tuningParams.aat_a_start      = buf[2];
        tuningParams.aat_a_stepWidth  = buf[3];
        tuningParams.aat_b_min        = buf[4];
        tuningParams.aat_b_max        = buf[5];
        tuningParams.aat_b_start      = buf[6];
        tuningParams.aat_b_stepWidth  = buf[7];
        tuningParams.phaTarget        = buf[8];
        tuningParams.phaWeight        = buf[9];
        tuningParams.ampTarget        = buf[10];
        tuningParams.ampWeight        = buf[11];
        tuningParams.doDynamicSteps   = buf[12];
        tuningParams.measureLimit     = buf[13] | (buf[14]<<8);

        timerStopwatchStart();
        err = st25r3916AatTune(&tuningParams, &tuningStatus);
        uint16_t duration = timerStopwatchMeasure();

        //
        txData[0] = (tuningStatus.aat_a);
        txData[1] = (tuningStatus.aat_b);
        txData[2] = (tuningStatus.pha);
        txData[3] = (tuningStatus.amp);
        txData[4] = 0;
    ...
    if (cmd == 0x20) /* Antenna measurement. */
    {
        st25r3916MeasureAmplitude(txData);
        st25r3916MeasurePhase(txData + 1);
        if (*txSize) *txSize = 2;
        err = RFAL_ERR_NONE;
    }
    if (cmd == 0x21) /* Get interrupt and result */
    { 
        err = RFAL_ERR_REQUEST; // was: processInterruptResult( buf, bufSize, txData, txSize);
    }
    if (cmd == 0x22) /* Turn on/off antenna */
    { 
        if (bufSize < 1) return (uint8_t)RFAL_ERR_PARAM;
        if( buf[0] != 0x00 )
        {
    ...
    if (cmd == 0x23) /* Get RFAL observation mode state */
    {
        rfalGetObsvMode( &txData[0], &txData[1] );
        if (*txSize) *txSize = 2;
        err = RFAL_ERR_NONE;
    }
    if (cmd == 0x24) /* Identify */
    {
      /* Direction: >> */
    ...
    if (cmd == 0x26) /* RFAL read analogue config */
    {
        err = rfalAnalogConfigListReadRaw(txData, *txSize, txSize);
    }
    if (cmd == 0x27) /* RFAL write analogue config */
    {
        rfalDeinitialize();
        err = rfalAnalogConfigListWriteRaw(buf, bufSize);
        rfalInitialize();
        rfalSetPreTxRxCallback( rfalPreTransceiveCb ); /* Restore callback cleared by rfalInitialize()*/
    }

    // ----------------------------------------------------------------------
    // - Group: Card emulation ----------------------------------------------
    // ----------------------------------------------------------------------
    if (cmd == 0x30) /* Process Card emulation */  
    {         
        err = processCardEmulation(rxData, rxSize, txData, txSize);
    }
    ...
    if (cmd == RFAL_CMD_BLOCKING_RX)
    {
        ...
        {
            RFAL_MEMCPY( (uint8_t*)&txData[2], gRxBuf, rfalConvBitsToBytes(gRcvdLen) );
        }
    ...
    if (cmd == RFAL_CMD_ISO14443_TXRX_ANTICOLLISION)
    {
        ...
        *txSize += rfalConvBitsToBytes( rcvdLen );
        RFAL_MEMCPY( (uint8_t*)&txData[4], (uint8_t*)&buf[6], buf[0] + rfalConvBitsToBytes( rcvdLen ) );
    }
    ...
    if (cmd == RFAL_CMD_ISO15693_TXRX_ANTICOLLISION_EOF)
    {
        ...
        err = rfalISO15693TransceiveEOFAnticollision( &txData[2], (uint8_t)*txSize, &rcvdLen );
    ...
    if (cmd == ISODEP_CMD_COMMANDS)
    {
        return(processIsoDep(rxData, rxSize, txData, txSize) );
    }

    if (cmd == RFAL_CHIP_CMD_SET_RFO){
        err = rfalChipSetRFO(rxData[1]);
    }
    if (cmd == RFAL_CHIP_CMD_GET_RFO){
        err = rfalChipGetRFO(&txData[0]);
        if (*txSize) *txSize = 1;
    }

    if (cmd == RFAL_CHIP_CMD_WAKEUP_START){
        rfalWakeUpConfig config;
        if (bufSize < 23) { *txSize = 0; return RFAL_ERR_PARAM;}
        config.period = (rfalWumPeriod)*buf++;
        config.irqTout = *buf++;
        config.swTagDetect = *buf++;
        config.indAmp.enabled = *buf++;
        config.indAmp.delta = *buf++;
        config.indAmp.fracDelta = *buf++;
        config.indAmp.reference = *buf++; // No need to set full 16 bits (internal usage)
        config.indAmp.autoAvg = *buf++;
        config.indAmp.aaInclMeas = *buf++;
        config.indAmp.aaWeight = (rfalWumAAWeight)*buf++;
        config.indPha.enabled = *buf++;
        config.indPha.delta = *buf++;
        config.indPha.fracDelta = *buf++;
        config.indPha.reference = *buf++; // No need to set full 16 bits (internal usage)
        config.indPha.autoAvg = *buf++;
        config.indPha.aaInclMeas = *buf++;
        config.indPha.aaWeight = (rfalWumAAWeight)*buf++;
        config.cap.enabled = *buf++;
        config.cap.delta = *buf++;
        config.cap.reference = *buf++; // No need to set full 16 bits (internal usage)
        config.cap.autoAvg = *buf++;
        config.cap.aaInclMeas = *buf++;
        config.cap.aaWeight = (rfalWumAAWeight)*buf++;
        err = rfalWakeUpModeStart( &config );
        *txSize = 0;
    }
    if (cmd == RFAL_CHIP_CMD_WAKEUP_HASWOKE){
        bool hasWoke = rfalWakeUpModeHasWoke( );
        txData[0] = hasWoke;
        if (*txSize) *txSize = 1;
    }
    if (cmd == RFAL_CHIP_CMD_WAKEUP_GETINFO){
        rfalWakeUpInfo info;
        if (bufSize < 1) { *txSize = 0; return RFAL_ERR_PARAM;}
        if (*txSize < 13){ *txSize = 0; return RFAL_ERR_REQUEST;}
        err = rfalWakeUpModeGetInfo( buf[0], &info );
        *txData++ = info.irqWut;
        *txData++ = info.indAmp.lastMeas;
        WRITE_VAL16_LE( info.indAmp.reference, uint16_t, txData);
        *txData++ = info.indAmp.irqWu;
        *txData++ = info.indPha.lastMeas;
        WRITE_VAL16_LE( info.indPha.reference, uint16_t, txData);
        *txData++ = info.indPha.irqWu;
        *txData++ = info.cap.lastMeas;
        WRITE_VAL16_LE( info.cap.reference, uint16_t, txData);
        *txData++ = info.cap.irqWu;
        *txSize = 13;
    }
    if (cmd == RFAL_CHIP_CMD_WAKEUP_STOP){
        err= rfalWakeUpModeStop( );
    }
    if (cmd == RFAL_CMD_DPO_GET_CURRENT_TABLE_ENTRY){
        if (* txSize < 1){ *txSize = 0; return RFAL_ERR_REQUEST;}
        * txData = rfalDpoGetCurrentTableIndex( );
        * txSize = 1;
        err      = RFAL_ERR_NONE;
    }
    if (cmd == RFAL_CMD_AC_SET){
        uint16_t ac;
        const char *              pDataIn = (const char *) buf;
        READ_VAL16_LE(ac, uint16_t           , pDataIn);
        err= rfalSetAnalogConfig(ac);
    }

    if (cmd == RFAL_CMD_DPO_READ){
        err = rfalDpoTableRead( (rfalDpoEntry*) &txData[1], rxData[1], &txData[0] );
    }
    if (cmd == RFAL_CMD_DPO_WRITE){
        uint8_t powerTableSize = rxData[1]; // Power table entries
        if(powerTableSize > 15){
            err = RFAL_ERR_PARAM;
        }else{
            err = rfalDpoTableWrite((rfalDpoEntry* )(rxData + 2), powerTableSize);
        }
    }
    if (cmd == RFAL_CMD_DPO_MEASURE_FUNC){
        switch(rxData[1]){
        default:
        case 0: rfalDpoSetMeasureCallback( rfalChipMeasureAmplitude ); break;
        case 1: rfalDpoSetMeasureCallback( rfalChipMeasurePhase ); break;
        }
        err = RFAL_ERR_NONE;
    }
    if (cmd == RFAL_CMD_DPO_ADJUST_AND_MEASURE){
      // if requested do adjust
      if(rxData[1] == 0){
          rfalDpoAdjust();
      }
      // current DPO entry
      memcpy(&txData[0], rfalDpoGetCurrentTableEntry(), sizeof(rfalDpoEntry));
      // amplitude, phase
      rfalChipMeasureAmplitude(&txData[0 + sizeof(rfalDpoEntry)]);
      rfalChipMeasurePhase(&txData[1 + sizeof(rfalDpoEntry)]);
      txData[2 + sizeof(rfalDpoEntry)] = 0;
      if (*txSize) *txSize = sizeof(rfalDpoEntry) + 3;
      err = RFAL_ERR_NONE;
    }
    
    if (cmd == RFAL_CMD_DPO_SET_ENABLED){
        rfalDpoSetEnabled(rxData[1] != 0);
        err = RFAL_ERR_NONE;
    }

    if (cmd == RFAL_CMD_DPO_IS_ENABLED){
        txData[0] = rfalDpoIsEnabled() ? 1 : 0;
        if (*txSize) *txSize = 1;
        err = RFAL_ERR_NONE;
    }

    if (cmd == RFAL_CMD_GET_TRANSCEIVE_RSSI){
        uint16_t rssi = 0;
        err = rfalGetTransceiveRSSI(&rssi);//(uint16_t*) &txData[0]);
        txData[0] = ((rssi>>8)&0xFF);
        txData[1] = ((rssi>>0)&0xFF);
        if (*txSize) *txSize = 2;
    }
...
void dispatcherWorker()
{
    rfalWorker();

    if (gIsoDepTransceiveOngoing == true)
    {
        gIsoDepTransceiveError = rfalIsoDepGetApduTransceiveStatus();
        if (RFAL_ERR_BUSY != gIsoDepTransceiveError)
        { /* if not busy anymore the transceive is supposed to be finished */
            gIsoDepTransceiveOngoing = false;
        }
    }
}
...
static ReturnCode processTopaz(const uint8_t *rxData, uint16_t rxSize, uint8_t *txData, uint16_t *txSize)
{
    ...
    RFAL_MEMSET(txData, 0xff, *txSize);
    ...
    switch (cmd)
    {
        ...
        case 0x9f:
            ...
            break;
        case 0x91:
            if (*txSize < 8) return RFAL_ERR_PARAM;
            ...
            if(RFAL_ERR_NOTSUPP == err)
                *txSize = 2;
            break;
        case 0x92:
            if (*txSize < 6) return RFAL_ERR_PARAM;
            err = topazReadUID(&card);
            ...
            break;
        case 0x93:
            if (bufSize < 4) { *txSize = 0; return RFAL_ERR_PARAM;}
            RFAL_MEMCPY(card.uid, buf, 4);
            err = topazReadAll(&card, txData, *txSize, txSize);
            break;
        case 0x94: /* FIXME: should be Read Byte */
            break;
        case 0x95:
            if (bufSize < 6) return RFAL_ERR_PARAM;
            RFAL_MEMCPY(card.uid, buf, 4);
            {
...
static ReturnCode processKovio(const uint8_t *rxData, uint16_t rxSize, uint8_t *txData, uint16_t *txSize)
{
    ...
    switch (cmd)
    {
        ...
        case 0x8f:
            ...
            break;
        case 0x81:
            if (*txSize < 32) return RFAL_ERR_PARAM;
            err = kovioRead(&card);
            if(err) *txSize = 0;
            else
            {
                RFAL_MEMCPY(txData, card.uid, card.length);
                *txSize = card.length;
            }
            break;
        default:
...
static ReturnCode processIso14443a(const uint8_t *rxData, uint16_t rxSize, uint8_t *txData, uint16_t *txSize)
{
    ...
    switch (cmd)
    {
        ...
        case 0xa1:
            ...
            if (*txSize < 17) return RFAL_ERR_PARAM;
            ...
            if(err == RFAL_ERR_NONE){
              ...
              if (perform_ac)
              {
                ...
                txData[5] = card.sak[1];
                txData[6] = card.sak[2];
                RFAL_MEMCPY(txData+7, card.uid, card.actlength);
        ...
        case 0xa4:
            if (bufSize < 2) return RFAL_ERR_PARAM;
            if (*txSize < 7) return RFAL_ERR_PARAM;
            {
                uint8_t fsdi = buf[0];
                uint8_t cid = buf[1];
        ...
        case 0xa7:
            ...
            {
                uint8_t actlen;
                err = t2tReadNBytes(buf[0], txData, *txSize, &actlen);
                *txSize = actlen;
            }
            break;
...
static ReturnCode processIso14443b(const uint8_t *rxData, uint16_t rxSize, uint8_t *txData, uint16_t *txSize)
{
    ...
    switch (cmd)
    {
        ...
        case 0xb2:
            RFAL_MEMCPY(card.pupi, &buf[0], ISO14443B_PUPI_LENGTH);
        ...
        case 0xb5:
            {
                ...
                RFAL_MEMCPY(txData, &stcard.Chip_ID, 1);
                *txSize = 1;
            }
            break;
        ...
        case 0xb7:
            {
                iso14443B_ST25TB_t stcard;
                uint8_t sn = buf[0];
                if(bufSize < 1) return RFAL_ERR_PARAM;
        ...
        case 0xb9:
            {
                ...
                *txSize = 0;
                ...
                {
                    *txSize = 8;
                    RFAL_MEMCPY(txData, &stcard.uid, ISO14443B_ST25TB_UIDSIZE);
        ...
        case 0xba:
            {
                iso14443B_ST25TB_t stcard;
                if (*txSize < 9) return RFAL_ERR_PARAM;
                *txSize = 0;
                ...
                if (RFAL_ERR_NONE == err)
                {
                    *txSize = 9;
                    RFAL_MEMCPY(txData, &stcard.Chip_ID, 1);
                    RFAL_MEMCPY(txData+1, stcard.uid, 8);
                    logUsart("ST25TB card found. UID: %s\n", hex2Str(stcard.uid, ISO14443B_ST25TB_UIDSIZE));
                }
            }
            break;
        ...
        case 0xbd:
            {
                ...
                if (*txSize < 4) return RFAL_ERR_PARAM;
                *txSize = 0;
                if(bufSize < 1) return RFAL_ERR_PARAM;
        ...
        case 0xbe:
            {
                uint8_t address = buf[0];
                if(bufSize < 5) return RFAL_ERR_PARAM;
...
/*!
  ...
  - #nfcRxNBytes() receives at most *txSize bytes from the other NFC device
    ...
    response:
    RFAL_ERR_NOMSG is returned as status in case there was no message.
    <table>
...
/*!
  ...
  - #iso15693Inventory()
    ...
    Response is:
    <table>
      <tr><th>   Byte</th><th>   0     </th><th>                1..10 </th><th>..</th><th>1+(num_cards*10)..10*(num_cards+1)</th></tr>
...
static iso15693ProximityCard_t cards[1];
static rfalNfcvListenDevice vCards[24];
static ReturnCode processIso15693(const uint8_t *rxData, uint16_t rxSize, uint8_t *txData, uint16_t *txSize)
{
    const uint8_t *buf     = rxData + 1;
    const uint16_t bufSize = rxSize - 1;
    ...
    {
        if (bufSize == 0) return RFAL_ERR_PARAM;
        if (bufSize >= 1+ISO15693_UID_LENGTH && !(buf[0]&0x10))
        {
            RFAL_MEMCPY(cards[0].uid, buf + 1, ISO15693_UID_LENGTH);
            cardptr = cards + 0;
            uid     = buf + 1; // Keep address of uid
        }
    }
    switch (cmd)
    {
        ...
        case 0xd3:
            {
                if (0xd2 == cmd)
                { /* NFC compliance mode does a 1 slot inventory in the beginning */
                    err = rfalNfcvPollerCollisionResolution(RFAL_COMPLIANCE_MODE_NFC, 24, vCards, &actcnt);
                }
                else
                { /* ISO compliance mode does immediately send 16 slot inventory */
                    err = rfalNfcvPollerCollisionResolution(RFAL_COMPLIANCE_MODE_ISO, 24, vCards, &actcnt);
                    // Enabling "WithSleep" would change state of the cards - unexpected for the user
                    // and only rarely finds more cards.
                    //err = rfalNfcvPollerCollisionResolutionWithSleep(24, vCards, &actcnt);
                }
                if (actcnt > 0)
                {
                    uint8_t i, *tx = txData;
                    *tx = actcnt;
                    ...
                    for (i = 0; i < actcnt; i++)
                    {
                        if (*txSize < (i+1) * RFAL_NFCV_UID_LEN + 1) 
                        {
                            i++;
                            break;
                        }
                        /* flags, dsfid, uid */
                        RFAL_MEMCPY(tx, &vCards[i].InvRes, RFAL_NFCV_UID_LEN + 2);
                        tx += RFAL_NFCV_UID_LEN + 2;

                        logUsart("%s, ", hex2Str(vCards[i].InvRes.UID, RFAL_NFCV_UID_LEN));
                    }
                    logUsart("\n");
                    *txSize = 1 + i * (RFAL_NFCV_UID_LEN + 2);
                    err = RFAL_ERR_NONE;
                }
                else
                {
        ...
        case 0xd4:
            if (bufSize < 9) return RFAL_ERR_PARAM;
        ...
        case 0xd6:
            if ( (bufSize < (1 + ISO15693_UID_LENGTH)) || (bufSize > (2 + ISO15693_UID_LENGTH)))
                return RFAL_ERR_PARAM;
            else
            {
                int mode = 0;
                if (bufSize == (1 + ISO15693_UID_LENGTH) ) 
                { // Regular case GetSystemInfo
                    err = iso15693GetPiccSystemInformation(cardptr, &sysInfo, &actLen);
                    if (RFAL_ERR_NONE == err)
                    {
                        if (*txSize > sizeof(sysInfo))
                            *txSize = sizeof(sysInfo);
                        RFAL_MEMCPY(txData, &sysInfo, *txSize);
                    }
                    else
                    {
                      *txSize = 0;
                ...
                {
                    mode = buf[ 1 + ISO15693_UID_LENGTH];
                }
                switch( mode)
                {
                    case 0: // Regular case GetSystemInfo
                        err = rfalNfcvPollerGetSystemInformation( RFAL_NFCV_REQ_FLAG_DEFAULT, uid, (uint8_t *) buffTmp, sizeof (buffTmp), &actLen);
                        if (RFAL_ERR_NONE == err)
                        {
                            if (*txSize > sizeof(sysInfo))
                                *txSize = sizeof(sysInfo);
                            RFAL_MEMCPY(txData, buffTmp, *txSize);
                        }
                        break;
                    case 1 : // Vicinity mode
                        err = rfalNfcvPollerGetSystemInformation( RFAL_NFCV_REQ_FLAG_DEFAULT | RFAL_NFCV_REQ_FLAG_PROTOCOL_EXT, uid, (uint8_t *) buffTmp, sizeof (buffTmp), &actLen);
            ...
            break;
        case 0xd7:
            if (bufSize < ISO15693_UID_LENGTH + 2) return RFAL_ERR_PARAM;
            if (*txSize < 2) return RFAL_ERR_PARAM;
            ...
            if (RFAL_ERR_NONE == err)
            {
                ...
                if (memBlock.errorCode == 0)
                {
                    ...
                    RFAL_MEMCPY(txData+1,memBlock.data,*txSize - 1);
                }
                else
                {
        ...
        case 0xd8:
            if (bufSize < ISO15693_UID_LENGTH + 3) return RFAL_ERR_PARAM;
            memBlock.actualSize = bufSize - ISO15693_UID_LENGTH - 2;
            memBlock.blocknr = buf[9];
            RFAL_MEMCPY((uint8_t*)memBlock.data, buf + 10, memBlock.actualSize);
...
static ReturnCode processDirectCommand(const uint8_t *rxData, uint16_t rxSize, uint8_t *txData, uint16_t *txSize)
{
    ...
    switch (cmd)
    {
        case ST25R3916_CMD_MEASURE_AMPLITUDE:
            err = st25r3916MeasureAmplitude(txData);
            if (*txSize) *txSize = 1;
            break;
        case ST25R3916_CMD_ADJUST_REGULATORS:
            { /* make sure MSB of voltage definition register is cleared */
                uint16_t mV;
                err = st25r3916ModifyRegister(ST25R3916_REG_REGULATOR_CONTROL, ST25R3916_REG_REGULATOR_CONTROL_reg_s, 0x0);
                st25r3916AdjustRegulators(&mV);
                txData[0] = mV & 0xff;
                txData[1] = mV >> 8;
                if (*txSize) *txSize = 2;
            }
            break;
        case ST25R3916_CMD_MEASURE_PHASE:
            err = st25r3916MeasurePhase(txData );
            if (*txSize) *txSize = 1;
            break;
#ifdef ST25R3916
        case ST25R3916_CMD_MEASURE_CAPACITANCE:                    // !!!! add methods
            err = st25r3916ExecuteCommandAndGetResult(cmd,
                                        ST25R3916_REG_AD_RESULT,
                                        100,
                                        txData);
            if (*txSize) *txSize = 1;
            break;
        case ST25R3916_CMD_CALIBRATE_C_SENSOR:
            err = st25r3916ExecuteCommandAndGetResult(cmd,
            ...
            break;
#endif
        case ST25R3916_CMD_MEASURE_VDD:
            {
                uint16_t val = 0;
                uint8_t mpsv = 0;
                if (rxSize > 1){
                ...
                {
                    err = st25r3916ReadRegister(ST25R3916_REG_REGULATOR_CONTROL, &mpsv);
                    mpsv &= ST25R3916_REG_REGULATOR_CONTROL_mpsv_mask;
                }

                if(mpsv == 0xFF){
                  // measure all
                  uint8_t i = 0;
                  val = st25r3916MeasureVoltage(ST25R3916_REG_REGULATOR_CONTROL_mpsv_vdd);    txData[i++] = val & 0xff; txData[i++] = val >> 8;
                  val = st25r3916MeasureVoltage(ST25R3916_REG_REGULATOR_CONTROL_mpsv_vdd_rf); txData[i++] = val & 0xff; txData[i++] = val >> 8;
                  val = st25r3916MeasureVoltage(ST25R3916_REG_REGULATOR_CONTROL_mpsv_vdd_a);  txData[i++] = val & 0xff; txData[i++] = val >> 8;
                  val = st25r3916MeasureVoltage(ST25R3916_REG_REGULATOR_CONTROL_mpsv_vdd_d);  txData[i++] = val & 0xff; txData[i++] = val >> 8;
                  val = st25r3916MeasureVoltage(ST25R3916_REG_REGULATOR_CONTROL_mpsv_vdd_am); txData[i++] = val & 0xff; txData[i++] = val >> 8;
                  txData[i++] = 0;  txData[i++] = 0;
                  if (*txSize) *txSize = i;
                  
                }else{
                
                  // measure requested value
                  switch(mpsv){
                    case ST25R3916_REG_REGULATOR_CONTROL_mpsv_vdd:
                    case ST25R3916_REG_REGULATOR_CONTROL_mpsv_vdd_rf:
                    case ST25R3916_REG_REGULATOR_CONTROL_mpsv_vdd_a:
                    case ST25R3916_REG_REGULATOR_CONTROL_mpsv_vdd_d:
                    case ST25R3916_REG_REGULATOR_CONTROL_mpsv_vdd_am:
                      val = st25r3916MeasureVoltage(mpsv);
                      break;
        ...
        default:
            err = st25r3916ExecuteCommand(cmd);
            *txSize = 0;
    }
...
static ReturnCode processFeliCa(const uint8_t *rxData, uint16_t rxSize, uint8_t *txData, uint16_t *txSize)
{
    ...
    switch (cmd)
    {
        ...
        case 0xF1:
            ...
            if (bufSize < 4) return RFAL_ERR_PARAM;
            if (*txSize < 2) return RFAL_ERR_PARAM;
            num_cards = txData;
            num_cols = txData + 1;
            ...
            *num_cols = 0;
            ...
            if ((*num_cards > 0)&&(RFAL_ERR_NONE == err)){
...
/*!
  Process card emulation commmands.
...
static ReturnCode processCardEmulation(const uint8_t * rxData, const uint16_t rxSize, uint8_t * txData, uint16_t *txSize)
{
    uint8_t ceCmd = rxData[1];
    ReturnCode err = RFAL_ERR_NONE;

    memset(txData, 0x0, *txSize);

    /* cardEmulation commands */
    switch (ceCmd)
    {
        case CARDEMULATION_CMD_START:
            txData[0] = ceStart(&rxData[2], rxSize - 2);
            *txSize = 1;
            break;

        case CARDEMULATION_CMD_STOP:
            txData[0] = ceStop();
            *txSize = 1;
            break;
        ...
        case CARDEMULATION_CMD_GET_RX_F:
            //
            *txSize = 0;
            err = txData[0] = ceGetRx(ceCmd, &txData[1], txSize);
            (*txSize)++;
            break;
        ...
        case CARDEMULATION_CMD_SET_TX_F:
            //
            err= txData[0] = ceSetTx(ceCmd, &rxData[2], rxSize - 2);
            *txSize = 1;
            break;

        case CARDEMULATION_CMD_GET_LISTEN_STATE:
            //
            *txSize = 0;
            txData[0] = ceGetListenState(&txData[1], txSize);
            (*txSize)++;
            break;
...
ReturnCode processDefault(const uint8_t *rxData, const uint16_t rxSize, uint8_t *txData, uint16_t *txSize)
{
    uint8_t    ceCmd = rxData[0];
    ReturnCode err   = RFAL_ERR_NONE;
    if (txSize)
    {
        * txSize =  0;
    ...
    switch (ceCmd)
    {
        case CMD_APPL_COM_WUPA:  /*!< = 0xE3  WUPB button pressed. */
            if (! rfalIsExtFieldOn() & 0)
            {
                err = RFAL_ERR_PROTO;
            }
            else
            {    /* Initialize for Type A */
                 rfalNfcaSensRes sensRes;
                 rfalNfcaPollerInitialize();
                 rfalSetErrorHandling(RFAL_ERRORHANDLING_NONE);
                 rfalFieldOnAndStartGT();    /* Field On and Start Tp */
                 rfalNfcaPollerCheckPresence(RFAL_14443A_SHORTFRAME_CMD_WUPA, & sensRes);
            }
             break;
        case CMD_APPL_COM_WUPB:  /*!< = 0xE4  WUPB button pressed. */
           if (!rfalIsExtFieldOn() & 0)
            {
                err = RFAL_ERR_PROTO;
            }
            else
            {                /* Initialize for Type B */
                rfalNfcbSensbRes  sensRes;
                uint8_t           sensResLen = 0;
                rfalNfcbPollerInitialize();
                rfalSetErrorHandling(RFAL_ERRORHANDLING_NONE);
                rfalFieldOnAndStartGT();  /* Field On and Start Tp */
                rfalNfcbPollerCheckPresence(RFAL_NFCB_SENS_CMD_ALLB_REQ, RFAL_NFCB_SLOT_NUM_1, &sensRes, &sensResLen);
            }
            break;
        default:
            err = RFAL_ERR_PARAM;
...
uint8_t applProcessCyclic ( uint8_t * protocol, uint16_t * txSize, uint8_t * txData, uint16_t remainingSize )
{
    ...
    counter++;
    *txSize = 0;
    return ST_STREAM_NO_ERROR; /* cyclic is always called, so it is no error if there is no function */
}

uint8_t applReadReg ( uint16_t rxSize, const uint8_t * rxData, uint16_t *txSize, uint8_t * txData)
{
    if(rxSize < 1) return (uint8_t)RFAL_ERR_REQUEST;

    if ( (rxData[0] & 0x80) )
    { /* virtual addresses above 0x3f provide access to test registers */
        rfalChipReadTestReg( (rxData[0] & 0x3F), txData );
    }
    else
    {
      rfalChipReadReg( rxData[0], txData, 1);
    }
...
uint8_t applWriteReg  ( uint16_t rxSize, const uint8_t * rxData, uint16_t *txSize, uint8_t * txData)
{
    ...
    if(rxSize < 2) return (uint8_t)RFAL_ERR_REQUEST;

    val  = rxData[1];

    if ( (rxData[0] & 0x80) )
    { /* virtual addresses above 0x3f provide access to test registers */
        rfalChipWriteTestReg( (rxData[0] & 0x3F), val );
        return 0;
    }

    rfalChipWriteReg( rxData[0], &val, 1);
    return 0;
}
...
//***************************************************************************************
// Isodep part
static int IsoDep_DeSerialize_Info( rfalIsoDepInfo * info, const uint8_t * * ptrR)
{
    int             size = 0;
    const uint8_t * start  = * ptrR; // Kepp start value
    READ_VAL08_LE( info->FWI     , uint8_t         , * ptrR);
    READ_VAL32_LE( info->FWT     , uint32_t        , * ptrR);
    READ_VAL32_LE( info->dFWT    , uint32_t        , * ptrR);
    READ_VAL32_LE( info->SFGI    , uint32_t        , * ptrR);
    READ_VAL32_LE( info->SFGT    , uint32_t        , * ptrR);
    READ_VAL08_LE( info->FSxI    , uint8_t         , * ptrR);
    READ_VAL16_LE( info->FSx     , uint16_t        , * ptrR);
    READ_VAL32_LE( info->MBL     , uint32_t        , * ptrR);
    READ_VAL08_LE( info->DSI     , rfalBitRate     , * ptrR);
    READ_VAL08_LE( info->DRI     , rfalBitRate     , * ptrR);
    READ_VAL08_LE( info->DID     , uint8_t         , * ptrR);
    READ_VAL08_LE( info->NAD     , uint8_t         , * ptrR);
    READ_VAL08_LE( info->supDID  , bool            , * ptrR);
    READ_VAL08_LE( info->supNAD  , bool            , * ptrR);
    READ_VAL08_LE( info->supAdFt , bool            , * ptrR);
    size = * ptrR - start; // Compute nb bytes read
    return(size);
}

static int IsoDep_Serialize_Info( const rfalIsoDepInfo * info, uint8_t * * ptrW)
{
    int       size = 0;
    uint8_t * start = * ptrW;
    WRITE_VAL08_LE( info->FWI     , uint8_t         , * ptrW);
    WRITE_VAL32_LE( info->FWT     , uint32_t        , * ptrW);
    WRITE_VAL32_LE( info->dFWT    , uint32_t        , * ptrW);
    WRITE_VAL32_LE( info->SFGI    , uint32_t        , * ptrW);
    WRITE_VAL32_LE( info->SFGT    , uint32_t        , * ptrW);
    WRITE_VAL08_LE( info->FSxI    , uint8_t         , * ptrW);
    WRITE_VAL16_LE( info->FSx     , uint16_t        , * ptrW);
    WRITE_VAL32_LE( info->MBL     , uint32_t        , * ptrW);
    WRITE_VAL08_LE( info->DSI     , rfalBitRate     , * ptrW);
    WRITE_VAL08_LE( info->DRI     , rfalBitRate     , * ptrW);
    WRITE_VAL08_LE( info->DID     , uint8_t         , * ptrW);
    WRITE_VAL08_LE( info->NAD     , uint8_t         , * ptrW);
    WRITE_VAL08_LE( info->supDID  , bool            , * ptrW);
    WRITE_VAL08_LE( info->supNAD  , bool            , * ptrW);
    WRITE_VAL08_LE( info->supAdFt , bool            , * ptrW);
    size = * ptrW - start;
    return(size);
}
...
static ReturnCode processIsoDep(const uint8_t *rxData, uint16_t rxSize, uint8_t * txData, uint16_t *txSize)
{
    uint16_t        cmd     = (uint16_t) rxData[0];
    ReturnCode      err     = RFAL_ERR_NONE;
    const uint8_t * buf     = rxData + 1;
    uint16_t        BuffInSize = rxSize - 1;
    uint8_t         len8;
    const uint16_t  OutSizeMax = * txSize;
    uint16_t        OutSize = 0;
    if (cmd == ISODEP_CMD_COMMANDS)
    { // Extension
        cmd =  ((uint16_t) rxData[1]) + ISODEP_CMD_EXTENSION1;
        buf        = rxData + 2;
        BuffInSize = rxSize - 2;
    }
    ...
    {
        OutSize =  sizeof(  uint32_t); // At least return ErrorCode
    }

    RFAL_MEMSET(txData, 0xff, * txSize);
    logUsart(" processIsoDep Cmd = %d: Input=%u chars output buf size=%u", cmd, rxSize, *txSize);

    err      = RFAL_ERR_PARAM; // by default
    * txSize = 0;

    switch (cmd)
    {
        case ISODEP_CMD_INITIALIZE:
            /* Configure demoboard for IsoDep. */
            rfalIsoDepInitialize();
            err     = RFAL_ERR_NONE;
            OutSize = 0;
            break;
        case ISODEP_CMD_FSXI2FSX: // uint16_t   rfalIsoDepFSxI2FSx ( uint8_t fsxi)
            OutSize = 2;
            if ( (BuffInSize == 1) && ( OutSizeMax >= OutSize) )
            {
                *((uint16_t *)txData) = rfalIsoDepFSxI2FSx (buf[0]);
                err     = RFAL_ERR_NONE;
            }
            break;
        case ISODEP_CMD_FWI2FWT: // uint32_t   rfalIsoDepFWI2FWT ( uint8_t fwi);
            OutSize = 4;
            if ( (BuffInSize == 1) && ( OutSizeMax >= OutSize) )
            {
                *((uint32_t *)txData) = rfalIsoDepFWI2FWT (buf[0]);
                err     = RFAL_ERR_NONE;
            }
            break;
        case ISODEP_CMD_PPS:
            ...
            break;
        case ISODEP_CMD_RATS:
            OutSize = sizeof(uint8_t)+ sizeof(rfalIsoDepAts);
            if ( (BuffInSize == 2) && ( OutSizeMax >= OutSize) )
            {   // ReturnCode rfalIsoDepRATS ( rfalIsoDepFSxI FSDI, uint8_t DID, rfalIsoDepAts * ats, uint8_t * atsLen)
                err = rfalIsoDepRATS ( (rfalIsoDepFSxI) buf[0],  buf[1] /*DID*/, (rfalIsoDepAts *) (txData + 1), (uint8_t *) (txData ));
                if (err == RFAL_ERR_NONE)
                { // Update size with relevant one, without compatibility bytes
                    OutSize = sizeof(uint8_t)+ *(uint8_t *) (txData );
                }
            }
            break;
        case ISODEP_CMD_POLLHANDLESPARAMETERS: 
            OutSize = ISODEP_DEVICE_INFO_STRUCT_SIZE;
            if ( (BuffInSize == (2U + ISODEP_DEVICE_INFO_STRUCT_SIZE) ) && ( OutSizeMax == OutSize) )
            { // Only the info part is used so only transfert in both sides this part
                rfalIsoDepDevice IsoDepDevice; // Must create an object to manage unaligned buffer
                const uint8_t *  ptrR = & buf[2];
                IsoDep_DeSerialize_Info( & IsoDepDevice.info, & ptrR);
                err = rfalIsoDepPollHandleSParameters ( & IsoDepDevice,  (rfalBitRate) buf[0] /*maxTxBR*/, (rfalBitRate) buf[1] /* maxRxBR*/ );
                if (  err == 0)
                {
                    uint8_t * ptrW = txData;
                    IsoDep_Serialize_Info( & IsoDepDevice.info, & ptrW);
                }
            }
            break;
        case ISODEP_CMD_POLLBHANDLEACTIVATION:
            OutSize = ISODEP_POLL_B_STRUCT_SIZE;
            err     =  RFAL_ERR_NOMEM;
            if ( (BuffInSize ==  0x14U ) /*4 + 4 + sizeof(rfalNfcbListenDevice) */
                && ( OutSizeMax == OutSize) )
            {
                const uint8_t* HLInfo = NULL;
                len8 =  * (buf + 4u + sizeof(rfalNfcbListenDevice) );  /* HLInfoLen */
                rfalIsoDepDevice   IsoDepDevice;
                if (len8 != 0)
                {
                     HLInfo = ( buf + 5 + sizeof(rfalNfcbListenDevice));  /* HLInfo */
                }
                err =  rfalIsoDepPollBHandleActivation (
                        (rfalIsoDepFSxI) buf[0] /*FSDI*/
                        , buf[1]                /*DID*/
                        , (rfalBitRate) buf[2]  /*maxBR*/
                        , buf[3]                /*PARAM1*/
                        , (rfalNfcbListenDevice *) (buf+4) /* nfcbDev*/
                        , HLInfo
                        , len8                  /* HLInfoLen */
                        , & IsoDepDevice
                        );
                uint8_t * ptr = txData;
                WRITE_VAL08_LE( IsoDepDevice.activation.B.Listener.ATTRIB_RES.mbliDid, uint8_t                      , ptr);
                WRITE_BUF     ( IsoDepDevice.activation.B.Listener.ATTRIB_RES.HLInfo , RFAL_ISODEP_ATTRIB_HLINFO_LEN, ptr);
                WRITE_VAL08_LE( IsoDepDevice.activation.B.Listener.ATTRIB_RESLen     , uint8_t                      , ptr);
                IsoDep_Serialize_Info( & IsoDepDevice.info, & ptr);
            }
            break;
        case ISODEP_CMD_INITIALIZEWITHPARAMS:
            OutSize = 0;
            if (BuffInSize == 7 )
            {
                rfalIsoDepInitializeWithParams
                    ( (rfalComplianceMode ) buf[0]
                    ,  buf[1] /*maxRetriesR*/
                    ,  buf[2] /*maxRetriesSnWTX*/
                    ,  buf[3] /*maxRetriesSWTX*/
                    ,  buf[4] /*maxRetriesSDSL*/
                    ,  buf[5] /*maxRetriesI*/
                    ,  buf[6] /*maxRetriesRATS*/);
                err     = RFAL_ERR_NONE;
            }
            break;
        case ISODEP_CMD_ATTRIB:
            OutSize = sizeof(uint8_t)+ sizeof(rfalIsoDepAttribRes);
            len8    =    buf[RFAL_NFCB_NFCID0_LEN + 6];  // param   HLInfoLen
            if ( (BuffInSize == (12 + len8)) && ( OutSizeMax >= OutSize) )
            {
                err = rfalIsoDepATTRIB (
                                      (uint8_t *)      buf                             /* nfcid0 */
                                    ,                  buf[RFAL_NFCB_NFCID0_LEN + 0]   /*PARAM1*/
                                    , (rfalBitRate)    buf[RFAL_NFCB_NFCID0_LEN + 1]   /* DSI */
                                    , (rfalBitRate)    buf[RFAL_NFCB_NFCID0_LEN + 2]   /* DRI */
                                    , (rfalIsoDepFSxI) buf[RFAL_NFCB_NFCID0_LEN + 3]   /* FSDI */
                                    , (uint8_t)        buf[RFAL_NFCB_NFCID0_LEN + 4]   /* PARAM3 */
                                    , (uint8_t)        buf[RFAL_NFCB_NFCID0_LEN + 5]   /* DID */
                                    , (const uint8_t *)buf + (RFAL_NFCB_NFCID0_LEN + 7)/* HLInfo*/
                                    , (uint8_t)        len8                            /* HLInfoLen */
                                    , (uint8_t)        buf[RFAL_NFCB_NFCID0_LEN + 7 + len8]   /* fwt */
                                    , (rfalIsoDepAttribRes *) txData + 1   /* attribRes */
                                    , (uint8_t *)      txData              /* attribResLen */
                );
            }
            break;
        case ISODEP_CMD_STARTAPDUTRANSCEIVE:
            OutSize = 0;
            if ( (BuffInSize >= 15) && ( OutSizeMax >= OutSize) )
            {
                const char *              pDataIn = (const char *) buf;
                rfalIsoDepApduTxRxParam   ApduParam;
                READ_VAL16_LE(ApduParam.txBufLen, uint16_t           , pDataIn);
                READ_VAL32_LE(ApduParam.FWT     , uint32_t           , pDataIn);
                READ_VAL32_LE(ApduParam.dFWT    , uint32_t           , pDataIn);
                READ_VAL16_LE(ApduParam.FSx     , uint16_t           , pDataIn);
                READ_VAL16_LE(ApduParam.ourFSx  , uint16_t           , pDataIn);
                READ_VAL08_LE(ApduParam.DID     , uint8_t            , pDataIn);
                ApduParam.txBuf    = (rfalIsoDepApduBufFormat *) pDataIn;
                ApduParam.rxBuf    = & gIsoDepApduBuffer;
                ApduParam.rxLen    = & gRcvdLen;
                ApduParam.tmpBuf   = (rfalIsoDepBufFormat *) gRxBuf; //We can use this buffer!
                pDataIn +=  ApduParam.txBufLen + RFAL_ISODEP_PROLOGUE_SIZE;
                err = rfalIsoDepStartApduTransceive ( ApduParam);
                gIsoDepTransceiveOngoing = true;
            }
            break;
        case ISODEP_CMD_POLLAHANDLEACTIVATION:
            OutSize = ISODEP_POLL_A_STRUCT_SIZE;
            err     =  RFAL_ERR_NOMEM;
            if ( (BuffInSize == 3) && ( OutSizeMax == OutSize) )
            {
                rfalIsoDepDevice IsoDepDevice;
                err = rfalIsoDepPollAHandleActivation
                        ...
                        , & IsoDepDevice);
                uint8_t * ptr = txData;
                WRITE_VAL08_LE( IsoDepDevice.activation.A.Listener.ATS.TL, uint8_t      , ptr);
                WRITE_VAL08_LE( IsoDepDevice.activation.A.Listener.ATS.T0, uint8_t      , ptr);
                WRITE_VAL08_LE( IsoDepDevice.activation.A.Listener.ATS.TA, uint8_t      , ptr);
                WRITE_VAL08_LE( IsoDepDevice.activation.A.Listener.ATS.TB, uint8_t      , ptr);
                WRITE_VAL08_LE( IsoDepDevice.activation.A.Listener.ATS.TC, uint8_t      , ptr);
                WRITE_BUF     ( IsoDepDevice.activation.A.Listener.ATS.HB, RFAL_ISODEP_ATS_HB_MAX_LEN, ptr);
                IsoDep_Serialize_Info( & IsoDepDevice.info, & ptr);
            }
            if (err != 0)
            {
            ...
            break;
        case ISODEP_CMD_DESELECT:
            OutSize = 0;
            if ( (BuffInSize == 0) && ( OutSizeMax >= OutSize) )
            {
                err = rfalIsoDepDeselect();
            }
            break;
#if RFAL_FEATURE_ISO_DEP_LISTEN0
        case ISODEP_CMD_LISTENGETACTIVATIONSTATUS:
            OutSize = 0;
            if ( (BuffInSize == 0) && ( OutSizeMax >= OutSize) )
            {
                err = rfalIsoDepListenGetActivationStatus();
            }
            break;
        case ISODEP_CMD_ISATTRIB:
            OutSize = 1;
            len8    =  buf[0];
            if ( (BuffInSize >= 1 ) && ( BuffInSize == len8 + 1) )
            {
                * txData = rfalIsoDepIsAttrib ( buf+1,  buf[0] /*bufLen*/);
                err     = RFAL_ERR_NONE;
            }
            break;
        case ISODEP_CMD_ISRATS:
            len8 =  buf[0];
            if ( (BuffInSize >= 1 ) && ( BuffInSize == len8 + 1) )
            {
                OutSize = 1;
                * txData = rfalIsoDepIsRats ( buf+1,  buf[0] /*bufLen*/);
                err      = RFAL_ERR_NONE;
            }
            break;
#endif //  RFAL_FEATURE_ISO_DEP_LISTEN0
        case ISODEP_CMD_GETAPDUTRANSCEIVESTATUS:
            OutSize = 0;
            err     = gIsoDepTransceiveError;
            // platformLog("\nrfalIsoDepStartApduTransceive Timer %d. Status = %d rcvLen=%d",  timerCalculateTimer( 0), * pErrorCode , gRcvdLen);
            if (err == RFAL_ERR_NONE)
            {
                uint16_t len = ((uint16_t) gRcvdLen) + RFAL_ISODEP_PROLOGUE_SIZE;
                OutSize =  sizeof(uint16_t) + len;
                if (OutSize <= OutSizeMax)
                {
                    uint8_t * ptr = txData;
                    WRITE_VAL16_LE( gRcvdLen           , uint16_t      , ptr);
                    WRITE_BUF     ( & gIsoDepApduBuffer, len           , ptr);
                }
                else
                {
                    err     = RFAL_ERR_NOMEM;
            ...
            break;
        case ISODEP_CMD_GETTRANSCEIVESTATUS:
            OutSize = 0;
            if ( (BuffInSize == 0) && ( OutSizeMax >= OutSize) )
            {
            ...
            break;
        case ISODEP_CMD_GETMAXINFLEN:
            OutSize = 2;
            if ( (BuffInSize == 0) && ( OutSizeMax >= OutSize) )
            {
                uint16_t res = rfalIsoDepGetMaxInfLen ();
                WRITE_VAL16_LE( res, uint16_t      , txData);
                err     = RFAL_ERR_NONE;
            }
            break;

         // Operation on Flash for persistent Analog Config
        case CMD_EXT_PERSISTENT_ANALOG_CONFIG:
            OutSize = 0;
            if ( (BuffInSize >= 1) && ( OutSizeMax >= OutSize) )
            {
                bool      status   = false;
                uint8_t * ptr      = txData;
                uint16_t  size;
                uint8_t   tmpbuf[RFAL_ANALOG_CONFIG_TBL_SIZE];
                err = RFAL_ERR_NONE;
                switch(   buf[0]) // buf[0] contains the command id 
                {
                    case PERSISTENT_ANALOG_CONFIG_WRITE:
                        // Install persistent
                        status = (persistentAnalogConfig_Program( ) == RFAL_ERR_NONE ? true : false);
                        break;
                    case PERSISTENT_ANALOG_CONFIG_CRC:
                    {
                        uint32_t  crcOriginal32;
                        uint32_t  crcStored32;
                        uint32_t  crcCurrent32;
                        // Retrieve current RFAL AC Table from RAM
                        rfalAnalogConfigListReadRaw( tmpbuf, sizeof(tmpbuf), & size );
                        // retreive now 32 bits CRCs
                        persistentAnalogConfig_GetHash(& crcOriginal32, & crcStored32);
                        // Retrieve current RFAL AC Table from RAM
                        crcCurrent32 = persistentAnalogConfig_CalcHash( tmpbuf, size );    // Compute checksum
                        WRITE_VAL32_LE( crcOriginal32, uint32_t , ptr);
                        WRITE_VAL32_LE( crcStored32  , uint32_t , ptr);
                        WRITE_VAL32_LE( crcCurrent32 , uint32_t , ptr);
                        WRITE_VAL32_LE( (CRC_32bits_FNV1a  | CRC_Original | CRC_Stored | CRC_Current), uint32_t , ptr);
                        OutSize = 16;
                        status  = true;
                        break;
                    }
                    case PERSISTENT_ANALOG_CONFIG_ERASE:
                        // status = persistentAnalogConfig_Erase();
                        err     = RFAL_ERR_NOT_IMPLEMENTED; // Should not erase in this version
                        break;
                    default:
                        status = true;
                        err     = RFAL_ERR_NOT_IMPLEMENTED; // Should not erase in this version
            ...
            break;
        default: // non implemented Isodep commands
            err     = RFAL_ERR_NOT_IMPLEMENTED;;
            break;
    }
    if ( err != RFAL_ERR_NONE)
    {
...
//***************************************************************************************
//***************************************************************************************
const uint32_t firmwareNumber = 0x010404;

const char * applFirmwareInformation(void)
{
#ifdef _NUCLEO_
  #ifdef ST25R3916B
    return "ST25R3916B-DISCO (NUCLEO) Firmware v1.4.4";
  #else
    return "ST25R3916-DISCO (NUCLEO) Firmware v1.4.4";
  #endif
#else
  #ifdef ST25R3916B
    return "ST25R3916B-DISCO (MB) v1.4.4";
  #else
    return "ST25R3916-DISCO (MB) v1.4.4";
  #endif
#endif
}
