 ...
 * 255 bytes: maximum size f�r the length byte + payload
 */
...
ReturnCode nfcInitialize(uint8_t is_active, uint8_t is_initiator, uint8_t bitrate, uint8_t listenModeMask)
{
    ...
    RFAL_MEMSET( nfcDepTagetParam.nfcid3, 0x00, RFAL_NFCDEP_NFCID3_LEN );
    ...
    RFAL_MEMCPY(lmConfA.nfcid, nfcid, RFAL_NFCID1_DOUBLE_LEN);
    ...
    RFAL_MEMCPY( lmConfF.SENSF_RES, sensfRes, sizeof(sensfRes) );
    ...
    else
    {
        ...
        nfcLmState = ((err == RFAL_ERR_NONE) ? NFC_LISTEN_WAIT_ATR : NFC_LISTEN_NONE);
    }


    return ( (RFAL_ERR_NONE != err) ? RFAL_ERR_IO : RFAL_ERR_NONE);
}
...
ReturnCode nfcDepTx( const uint8_t *buf, uint16_t bufSize )
{
    ...
    /* Copy data to send */
    RFAL_MEMCPY( nfcDepTxBuf.inf, buf, MIN( bufSize, RFAL_NFCDEP_FRAME_SIZE_MAX_LEN ) );
    ...
    switch( ret )
    {
        ...
        case RFAL_ERR_NONE:
            *actlen = MIN( *rfalNfcDepTxRx.rxLen, bufsize );
            RFAL_MEMCPY( buf, &rfalNfcDepTxRx.rxBuf->inf, *actlen );
            break;
...
ReturnCode nfcDepInitiatorHandleActivation( uint8_t* nfcid3, uint8_t desiredBitRate, uint8_t gbLen, uint8_t* gb, uint8_t* currentBitRate, uint8_t* atrResLen, uint8_t *atrRes )
{
    ...
    if( ret == RFAL_ERR_NONE )
    {
        ...
        *atrResLen       = nfcDepDev.activation.Target.ATR_RESLen;
        RFAL_MEMCPY(atrRes, &nfcDepDev.activation.Target.ATR_RES, nfcDepDev.activation.Target.ATR_RESLen);
    }
...
void nfcDepTargetSetParams( uint8_t *nfcid, uint8_t lr, uint8_t gbLen, uint8_t *gb )
{
    ...
    RFAL_MEMCPY( nfcDepTagetParam.nfcid3, nfcid, RFAL_NFCDEP_NFCID3_LEN );
    RFAL_MEMCPY( nfcDepTagetParam.GBt, gb, MIN(gbLen, RFAL_NFCDEP_GB_MAX_LEN) );
}
...
ReturnCode NfcDepTargetHandleActivation( uint8_t *bitrate, bool *isActive, uint8_t *atrReqLen, uint8_t *atrReq )
{
    ...
    switch( nfcLmState )
    {
        case NFC_LISTEN_WAIT_ATR:
            ...
            if( ((lmState == RFAL_LM_STATE_IDLE) || (lmState == RFAL_LM_STATE_READY_F) || (lmState == RFAL_LM_STATE_ACTIVE_A) || (lmState == RFAL_LM_STATE_ACTIVE_Ax)) && (dataFlag == true) && (nfcRxBufferActLength > 0))
            {
                ...
                if( rfalNfcDepIsAtrReq( (nfcRxBuffer+dataHdr), dataLen, NULL ) )
                {
                    ...
                    else if( (lmState == RFAL_LM_STATE_ACTIVE_A) || (lmState == RFAL_LM_STATE_ACTIVE_Ax) ) /* Passive NFC-A P2P */
                    {
                        nfcDepTagetParam.commMode = RFAL_NFCDEP_COMM_PASSIVE;
