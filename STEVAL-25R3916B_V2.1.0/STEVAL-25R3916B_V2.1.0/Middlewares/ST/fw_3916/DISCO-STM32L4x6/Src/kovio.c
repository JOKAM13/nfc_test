...
ReturnCode kovioRead(kovioProximityCard_t *card)
{
    ...
    { /* Scan two times for ID, since we might start receiving in the middle of a transmission */
        ...
        RFAL_EXIT_ON_ERR(err, rfalTransceiveBlockingTx( NULL, 0, card->uid, KOVIO_UID_LENGTH, &actrxlength, (RFAL_TXRX_FLAGS_DEFAULT | RFAL_TXRX_FLAGS_CRC_RX_KEEP | RFAL_TXRX_FLAGS_PAR_RX_KEEP | RFAL_TXRX_FLAGS_CRC_RX_MANUAL), rfalConvMsTo1fc(5) ));
        err = rfalTransceiveBlockingRx();  /* Get rcvd length in bits */

        if ( (RFAL_ERR_TIMEOUT != err) && ((127 == actrxlength) || (255 == actrxlength)) )
    ...
    if ( RFAL_ERR_INCOMPLETE_BYTE == err )
    {
        err = RFAL_ERR_NONE;
    }
    ...
    if ( (32 != card->length) && (16 != card->length) )
    {
