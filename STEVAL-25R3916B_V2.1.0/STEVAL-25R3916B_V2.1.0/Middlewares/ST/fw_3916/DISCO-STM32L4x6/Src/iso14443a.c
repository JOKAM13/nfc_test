...
ReturnCode iso14443ASelect(iso14443ACommand_t cmd, iso14443AProximityCard_t* card, uint8_t perform_anticollision)
{
    ...
    if (RFAL_ERR_NONE==err && 0x00 == (card->atqa[0] & 0x1f)) /* Select/anticollision not supported */
    {
        return RFAL_ERR_NOTSUPP; /* Select/anticollision not supported */
    }
    ...
    if (RFAL_ERR_TIMEOUT == err)
    {
        err = RFAL_ERR_NOTSUPP; /* Select/anticollision not supported */
    }
...
ReturnCode iso14443ASendProtocolAndParameterSelection(uint8_t cid, uint8_t pps1)
{
    ...
    /* map timeout error to PICC not found */
    if ((RFAL_ERR_TIMEOUT == err) || ((ISO14443A_CMD_PPSS | cid) != ppsRes.PPSS))
    {
        err = RFAL_ERR_NOTFOUND;
    }
