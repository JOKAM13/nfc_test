...
{
  const uint32_t screencaptureaddress = 0x080D0000; /* Needing 0x25800 bytes in flash, (height * (width * sizeof uint16_t)) => 240 * (320 * 2)) */
  uint32_t width = BSP_LCD_GetXSize();
  uint32_t height = BSP_LCD_GetYSize();

  /* Lowered the SPI freq to enable reads */
  BSP_LCD_LowFreq();

  /* Preparing the flash */
  FLASH_If_FlashUnlock( );
#if defined(ST25DX_DISCOVERY_MB1283)
  FLASH_If_EraseSectors( screencaptureaddress, FIRMWARE_FLASH_LAST_PAGE_ADDRESS );
#elif defined(ST25_DISCOVERY_MB1396)
  FLASH_If_PageErase( screencaptureaddress, FIRMWARE_FLASH_LAST_PAGE_ADDRESS );
#endif /* ST25DX_DISCOVERY_MB1283 */

  /* Storing LCD screen content to flash memory, be patient it may take a while */
  uint16_t buffer[320];
...
#if defined(ST25DX_DISCOVERY_MB1283)
    FLASH_If_WriteBuffer( (screencaptureaddress + (y * sizeof(buffer))), FIRMWARE_FLASH_LAST_PAGE_ADDRESS, (uint8_t *)buffer, sizeof(buffer) );
#elif defined(ST25_DISCOVERY_MB1396)
    uint64_t *cast_buffer = (uint64_t *)buffer;
    FLASH_If_WriteBuffer( (screencaptureaddress + (y * sizeof(buffer))), cast_buffer, sizeof(buffer) / 8 );
#endif /* ST25DX_DISCOVERY_MB1283 */
  }
  
  /* Setting SW BreakPoint to export data stored into flash */
  /* Using the following command, compiling without optimization is better for correct operation. Otherwise, use the direct memory address instead of variable */
  /* Keil : Run "save LCDscreen.hex screencaptureaddress,(screencaptureaddress+(height * (width * sizeof(uint16_t))))" in debug commnand window to save data */
  /* STM32CubeIDE : Run "dump ihex memory LCDscreen.hex screencaptureaddress (screencaptureaddress+(height * (width * sizeof(uint16_t))))" in Debugger Console window to save data */
  /* Uncomment following line for Sw Breakpoint in debug mode (or add a Hw Breakpoint instead) */
  //__BKPT(0);

  /* Resetting the SPI freq for write only */
  BSP_LCD_HighFreq();
