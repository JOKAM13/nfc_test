...
/** @addtogroup BSP Board Support Package
  ...
  * @details  The Board Support Package software (BSP) is defined by the Cube methodology as the abstraction layer 
  *           for the board specific features.\n
...
/** @defgroup ST25_DISCOVERY_NFCTAG ST25 Discovery NFCTAG Board Support Package
  * @brief    This module provides high-level functions to access the ST25DV-I2C NFC dynamic tag.
...
static int32_t BSP_ST25DV_Probe(void);
static int32_t BSP_ST25DVxxKC_Probe(void);
...
{
  if(BSP_ST25DV_Probe() == BSP_ERROR_NONE) 
  { 
    return NFCTAG_OK;
  }
  
  if(BSP_ST25DVxxKC_Probe() == BSP_ERROR_NONE) 
  { 
    return NFCTAG_OK;
  }
...
static int32_t BSP_ST25DV_Probe(void)
{
  ...
  IO.Init         = NFCMEM_IO_Init;
  IO.DeInit       = NFCMEM_IO_DeInit;
  IO.IsReady      = NFCMEM_IO_IsDeviceReady;
  IO.Read         = NFCMEM_IO_MemRead;
  IO.Write        = (ST25DV_Write_Func)NFCMEM_IO_MemWrite;
...
static int32_t BSP_ST25DVxxKC_Probe(void)
{
  ...
  IO.IsReady      = NFCMEM_IO_IsDeviceReady;
  IO.Read         = NFCMEM_IO_MemRead;
  IO.Write        = (ST25DVxxKC_Write_Func)NFCMEM_IO_MemWrite;
  IO.GetTick      = NFCMEM_IO_Tick;
...
{ 
  ...
  {
    Nfctag_Drv = NULL;
    ST25DVObj.IsInitialized = 0;
    ST25DVxxKCObj.IsInitialized = 0;
  }
}
...
int32_t BSP_NFCTAG_isInitialized(const uint32_t Instance)
{
  ...
  status = ST25DVObj.IsInitialized | ST25DVxxKCObj.IsInitialized;
...
{
  UNUSED(Instance);
  int32_t ret_value = NFCTAG_ERROR;
...
{
  UNUSED(Instance);
  uint32_t ret_value = NFCTAG_ERROR;
  ...
  if(ST25DVObj.IsInitialized == 1)
  {
    ...
    ret_value = (mem_size.BlockSize + 1) * (mem_size.Mem_Size + 1);
  }
  
  if(ST25DVxxKCObj.IsInitialized == 1)
  {
    ST25DVxxKC_ReadMemSize(NfcTagObj, &mem_size_xxkc);
    ret_value = (mem_size_xxkc.BlockSize + 1) * (mem_size_xxkc.Mem_Size + 1);
  }
...
/**
  * @brief  Reads the ST25DV-I2C IC Revision.
  * @param  pICRev Pointer on the uint8_t used to return the ST25DV-I2C IC Revision number.
...
/**
  * @brief  Reads the ST25DV-I2C ITtime duration for the GPO pulses.
  * @param  pITtime Pointer to return the GPO pulse duration coefficient = 302,06 us - ITtime * 512 / fc.
  * @return int32_t enum status.
  */
...
/**
  * @brief    Configures the ST25DV-I2C ITtime duration for the GPO pulse.
...
/**
  * @brief  Reads the ST25DV-I2C UID.
  * @param  pUid Pointer used to return the ST25DV-I2C UID value.
  * @return int32_t enum status.
  */
...
/**
  * @brief  Reads the ST25DV-I2C DSFID.
  * @param  pDsfid Pointer used to return the ST25DV-I2C DSFID value.
  * @return int32_t enum status.
  */
...
/**
  * @brief  Reads the ST25DV-I2C DSFID RF Lock state.
...
/**
  * @brief  Reads the ST25DV-I2C AFI.
  * @param  pAfi Pointer used to return the ST25DV-I2C AFI value.
  * @return int32_t enum status.
  */
