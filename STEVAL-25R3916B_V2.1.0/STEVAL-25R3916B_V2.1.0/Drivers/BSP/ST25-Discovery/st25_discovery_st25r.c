...
static rfalNfcDevice *Current = NULL;
/* Hold current protocol set */
static BSP_NFCTAG_Protocol_t *CurrentProtocol = NULL;
static bool NdefSupportA = false;
static bool NdefSupportB = false;
static bool NdefSupportF = false;
static bool NdefSupportV = false;
...
static ReturnCode t4tSendCommand(uint8_t* txBuf, uint16_t txLen, rfalIsoDepBufFormat* rxBuf, uint16_t* rxBufLen)
{
  ...
  } while ((err == RFAL_ERR_NONE) && *isoDepTxRx.isRxChaining);
  return err;
}
...
uint8_t BSP_NFCTAG_T4_SelectFile(uint16_t fileId)
{
    ...
    if( (err != RFAL_ERR_NONE) || (rxBuf.inf[0] != 0x90) || (rxBuf.inf[1] != 0x00))
    {
        return NFCTAG_ERROR;
    }
...
static uint8_t BSP_NFCTAG_Activate_NfcA(void)
{
    ...
    if(Current->type != RFAL_NFC_LISTEN_TYPE_NFCA)
    {
      return NFCTAG_ERROR;
    }

    // By default: claim that Ndef is not supported
    *CurrentProtocol->NdefSupport = false;
    ...
    if(Current->dev.nfca.type != RFAL_NFCA_T1T)
    {
      ...
      if(err == RFAL_ERR_NONE)
      {
          ...
          *CurrentProtocol->NdefSupport = true;
          return NFCTAG_OK;
      }
    }
          ...
          case RFAL_NFCA_T2T:
                {
                  ...
                  if(status >= RFAL_ERR_INCOMPLETE_BYTE)
                    status = RFAL_ERR_NONE;
                  if(status != RFAL_ERR_NONE)
                    return NFCTAG_ERROR;
...
static uint8_t BSP_NFCTAG_Activate_NfcB(void)
{
  ...
  *CurrentProtocol->NdefSupport = false;
  ...
  rfalFieldOnAndStartGT();    /* Turns the Field On and starts GT timer */
  if(Current->type == RFAL_NFC_LISTEN_TYPE_ST25TB)
  {
  ...
  if(Current->type == RFAL_NFC_LISTEN_TYPE_NFCB)
  {
    rfalIsoDepBufFormat rxBuf;
    uint16_t rxBufLen = sizeof(rxBuf);
...
static uint8_t BSP_NFCTAG_ReadData_NfcB(uint8_t* buffer, uint32_t offset, uint32_t length)
{
  ...
  } else if (Current->type == RFAL_NFC_LISTEN_TYPE_NFCB) {
      uint8_t rxBuf[RFAL_ISODEP_DEFAULT_FSC + RFAL_ISODEP_PROLOGUE_SIZE];
      uint16_t rxBufLen = 0xF0;
      uint8_t err;
      // read bytes
...
static uint8_t BSP_NFCTAG_Activate_NfcF(void)
{
  ...
  if(CurrentProtocol->read_data(testBuf, 0, 16) != RFAL_ERR_NONE)
  {
    *CurrentProtocol->NdefSupport = false;
  }
  else
...
uint8_t BSP_NFCTAG_CheckVicinity()
{
  ...
  if(Current->type != RFAL_NFC_LISTEN_TYPE_NFCV)
  {
    return 0;
  }
...
static uint8_t BSP_NFCTAG_Activate_NfcV(void)
{
  if(Current->type == RFAL_NFC_LISTEN_TYPE_NFCV)
  {
    ...
    status = rfalNfcvPollerExtendedGetSystemInformation( (RFAL_NFCV_REQ_FLAG_DEFAULT | RFAL_NFCV_REQ_FLAG_ADDRESS), Current->dev.nfcv.InvRes.UID, 0x30, rxBuf, 16, &rcvlen );
    if(status == RFAL_ERR_NONE)
    {
...
static uint8_t BSP_NFCTAG_ReadData_NfcV(uint8_t* buffer, uint32_t offset, uint32_t length)
{
  ...
  if((Current->type != RFAL_NFC_LISTEN_TYPE_NFCV) || (length == 0))
  {
    return NFCTAG_ERROR;
  }
  ...
  while(i < length)
  {
    ...
    if ((status != RFAL_ERR_NONE) || (!t5MultipleBlockSupport))
    {
    ...
    if ((status != RFAL_ERR_NONE) || (nbBytesRead < (1 + (offset+i)%4)))
      return NFCTAG_ERROR;
...
static uint8_t BSP_NFCTAG_WriteData_NfcV(uint8_t* buffer, uint32_t offset, uint32_t length)
{
  ...
  while(currentOffset < lastOffset)
  {
    ...
    if((blockInternalOffset) || (blockInternalLength%4))
    {
      ...
      if((status != RFAL_ERR_NONE) || (block[0] != 0))
      {
        return NFCTAG_ERROR;
      }
      // remove first byte (tag status)
...
/**
  * @brief Generic method to get the tag NDEF support status
  * @returns true for NDEF supported, otherwise fales
  */
bool BSP_NFCTAG_GetNdefSupport(void)
{
  if((CurrentProtocol != NULL) && (CurrentProtocol->NdefSupport != NULL))
  {
    return *CurrentProtocol->NdefSupport;
  }
  else
  {
    return false;
